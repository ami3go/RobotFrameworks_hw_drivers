"""RFDS-008 evidence and diagnostics engine for :mod:`rf_ngi_n83624`.

This driver already has a lightweight opt-in JSONL audit log
(``audit_log_path`` on every ``Open N83624 * Connection`` keyword, implemented
by ``NGI_N83624._audit_for`` in ``library.py``) — one line per safety-relevant
action, no correlation IDs, no protocol trace, no integrity hashing, off by
default. This module is a separate, deeper, always-on-by-default layer: every
public keyword call becomes a correlated operation record (arguments,
duration, result/failure) plus a protocol trace of the actual SCPI
write/query traffic, written to ``results/session/rf_ngi_n83624/<run>/`` with
a SHA-256-hashed manifest. The two systems are independent and can both be
active at once; see ``docs/logging_and_evidence.md`` for how they relate.

One :class:`EvidenceRun` exists per open N83624 session (keyed by connection
alias in ``NGI_N83624._evidence_runs``, mirroring ``NGI_N83624._sessions``),
since this library supports multiple simultaneous named connections
(``ROBOT_LIBRARY_SCOPE = "GLOBAL"``). A run is created when its alias first
appears (usually at ``Open N83624 * Connection``) and finalized when that
alias's session actually closes (``Close N83624 Connection``/``Close All
N83624 Connections``); see ``NGI_N83624._finalize_closed_sessions``.

Protocol tracing works through ``ngi_n83624.driver.N83624CellSimulator.protocol_observer``
— an optional ``(direction, text)`` callback the core driver invokes for
every SCPI write/query, added specifically so this framework-layer module can
observe the wire protocol without the core driver package knowing anything
about RFDS-008, evidence, or Robot Framework (see the hook's docstring in
...
redacted, integrity-hashed evidence — and deliberately does not implement
platform-scale concerns the standard also describes (log rotation/
backpressure policy, crash-recovery tooling, cryptographic signing, a shared
cross-driver package — none exists yet in this repository, see RFDS-008 §35).
RFDS-007 error codes are approximated from this driver's own exception
hierarchy (``ngi_n83624/exceptions.py``) since RFDS-007 itself was not
...
logger = logging.getLogger("rf_ngi_n83624.evidence")
...
def _classify_exception(exc: BaseException) -> str:
    ...
    ``ngi_n83624/exceptions.py`` names (``ValidationError``, ``SafetyError``,
    ``SessionStateError``, ``CommunicationError``, ``TimeoutError``,
    ``ProtocolError``, ``HeartbeatError``, ``RecoveryError``) are mapped to a
    small stable category set — not literal RFDS-007 codes (see module
    docstring).
    """
    name = type(exc).__name__
    if name in ("ValidationError",):
        return "VALIDATION"
    if name in ("SafetyError", "SessionStateError"):
        return "STATE"
    if name in ("CommunicationError", "TimeoutError", "ProtocolError", "HeartbeatError", "RecoveryError"):
        return "HARDWARE"
...
class EvidenceRun:
    """Owns one RFDS-008 result directory for one N83624 connection alias."""
    ...
    def _write_environment(self) -> None:
        payload = {
            ...
            "driver_distribution_version": _safe_distribution_version("rf-ngi-n83624"),
            "driver_source_version": _safe_module_version("rf_ngi_n83624"),
    ...
    def emit_event(
        ...
        *,
        ...
        record = {
            ...
            "schema_version": SCHEMA_VERSION,
            "run_id": self.run_id,
            "producer_id": f"rf_ngi_n83624:{session_alias or self.driver_id}",
            "timestamp_utc": _utc_now_iso(),
        ...
        # ERROR/CRITICAL are not counted here — every current ERROR/CRITICAL
        # emit_event call is paired with a record_error() call, which is the
        # sole owner of _error_count (avoids double-counting one failure).
    ...
    def log_protocol(
        ...
        *,
        ...
        """Record one outbound/inbound SCPI exchange (see ``protocol_observer``)."""
    ...
    def _write_markdown_summary(self, summary: dict) -> None:
        lines = [
            ...
            "for the underlying SCPI write/query traffic, in the order they happened.",
            "",
        ]
...
class EvidenceListener:
    """Register with ``--listener rf_ngi_n83624.evidence.EvidenceListener`` to
