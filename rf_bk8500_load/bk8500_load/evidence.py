"""RFDS-008 live evidence and diagnostics engine for :mod:`bk8500_load`.

This is a per-keyword-call, always-on evidence system, distinct from the
one-off archived conformance record already checked into
``evidence/hardware_conformance/v26.14_com12_2026-07-28/`` (a post-hoc summary
generated from one completed ``output.xml``, see that directory's own
``README.md``). This module instead builds a fresh, timestamped run under
``results/session/bk8500_load/<run>/`` for *any* use of the library — one
JSON Lines entry per event/operation/error, a SHA-256-hashed manifest, and a
protocol trace of every 26-byte frame this driver sends and receives (see
``bk8500_load/protocol.py`` for the frame format). It complements rather than
replaces the module-level ``logging.getLogger(__name__)`` calls already in
``driver.py``/``transport.py``.

Because :class:`~bk8500_load.library.BK8500Library` is ``ROBOT_LIBRARY_SCOPE
= "GLOBAL"`` and supports several simultaneously-open aliased connections,
there is exactly one :class:`EvidenceRun` per library instance (not one per
alias) — operations from different aliases are tagged with different
``session_alias`` values within that one run, rather than each alias getting
its own run directory. The run is created lazily on first use and finalized
when the library becomes fully disconnected (``Close Load Connection``
closing the last open alias, or ``Close All Load Connections``).

Scope note: this module implements the parts of RFDS-008 that make a concrete
troubleshooting difference for this driver (structured, correlated, redacted,
hash-verified evidence) — not the full platform-scale standard (log
rotation/backpressure policy, cryptographic signing, retention automation, a
shared cross-driver package: none exists yet in this repository). See
...
logger = logging.getLogger("bk8500_load.evidence")
...
def _classify_exception(exc: BaseException) -> str:
    ...
    RFDS-008 §17 calls for RFDS-007 error codes; RFDS-007 was not available
    while writing this, so this maps ``bk8500_load/exceptions.py`` class
    names to a small stable category set instead of inventing RFDS-007 codes.
    """
    name = type(exc).__name__
    if "Connection" in name or "State" in name:
        return "STATE"
    if "Configuration" in name or "Validation" in name or "Safety" in name or "ArgumentType" in name:
        return "VALIDATION"
    if "Timeout" in name or "Protocol" in name or "Command" in name or "Protection" in name:
        return "HARDWARE"
...
def _safe_module_version(module_name: str, attr: str = "__version__") -> str | None:
    try:
...
class EvidenceRun:
    """Owns one RFDS-008 result directory for one ``BK8500Library`` instance."""
    ...
    def _write_environment(self) -> None:
        payload = {
            ...
            "driver_distribution_version": _safe_distribution_version("bk8500-load"),
            "driver_source_version": _safe_module_version("bk8500_load", "VERSION")
            or _safe_module_version("bk8500_load.version", "VERSION"),
    ...
    def emit_event(
        ...
        *,
        ...
        record = {
            ...
            "schema_version": SCHEMA_VERSION,
            "run_id": self.run_id,
            "producer_id": f"bk8500_load:{session_alias or self.driver_id}",
            "timestamp_utc": _utc_now_iso(),
        ...
        # ERROR/CRITICAL is not counted here: it's always paired with a
        # record_error() call, which is the sole owner of _error_count.
    ...
    def log_protocol(
        ...
        *,
        ...
        """Record one outbound/inbound 26-byte frame as protocol evidence.

        ``frame_hex`` carries the raw frame bytes losslessly as hex (RFDS-008
        §24.1: binary protocol data shall be preserved losslessly as binary
        attachments or a documented representation such as hex/Base64).
        ``operation_text`` carries the human-readable decode (command name,
        address, payload) already produced by ``protocol.format_frame``.
        """
    ...
    def record_device_identity(self, *, session_alias: str, **fields: Any) -> None:
        """Record identity for one aliased connection (this driver supports several at once)."""
        self._device_identity[session_alias] = {k: v for k, v in fields.items() if v is not None}
        payload = {
    ...
    def _write_markdown_summary(self, summary: dict) -> None:
        lines = [
            ...
            "call this run made (across every open connection alias), `events/errors.jsonl` for",
            "failures, and `protocol/exchanges.jsonl` for the individual 26-byte frames exchanged.",
            "",
        ]
...
class EvidenceListener:
    """Register with ``--listener bk8500_load.evidence.EvidenceListener`` to
    correlate events with the active Robot suite/test/keyword. Optional."""
