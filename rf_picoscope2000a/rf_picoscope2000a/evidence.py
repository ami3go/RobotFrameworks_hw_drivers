"""RFDS-008 evidence and diagnostics engine for :mod:`rf_picoscope2000a`.

Every public keyword call is recorded as a *run* under ``results/session/
rf_picoscope2000a/<timestamp>_<run_id>/`` (override the root with the
...
rf_phidget_relay — this driver wraps a vendor SDK (``picosdk.ps2000a``) the
same way rf_phidget_relay wraps Phidget22, not a text transport like
...
rf_picoscope2000a.evidence.EvidenceListener`` to correlate events with the
...
logger = logging.getLogger("rf_picoscope2000a.evidence")
...
def _classify_exception(exc: BaseException) -> str:
    ...
    (``picoscope2000a/exceptions.py``) to a small stable category set instead
...
class EvidenceRun:
    ...
    def _write_environment(self) -> None:
        payload = {
            ...
            "driver_distribution_version": _safe_distribution_version("robotframework-picoscope2000a"),
            "driver_source_version": _safe_module_version("rf_picoscope2000a"),
    ...
    def emit_event(
        ...
        *,
        ...
        record = {
            ...
            "schema_version": SCHEMA_VERSION,
            "run_id": self.run_id,
            "producer_id": f"rf_picoscope2000a:{session_alias or self.driver_id}",
            "timestamp_utc": _utc_now_iso(),
...
class EvidenceListener:
    """Register with ``--listener rf_picoscope2000a.evidence.EvidenceListener``.
