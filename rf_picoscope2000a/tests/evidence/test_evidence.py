...
Scoped to what rf_picoscope2000a's evidence.py actually produces — this is
...
from picoscope2000a.exceptions import PicoScope2000AValidationError
from rf_picoscope2000a import evidence as evidence_module
from rf_picoscope2000a.library import PicoScope2000ALibrary
...
def _run_root(tmp_path) -> Path:
    session_root = tmp_path / "results" / "session" / "rf_picoscope2000a"
...
def test_end_suite_finalizes_a_complete_evidence_run(scope):
    ...
    assert summary["driver_id"] == "rf_picoscope2000a"
...
def test_jsonl_streams_are_valid_and_gap_free(scope):
    ...
    except PicoScope2000AValidationError:
        pass
    lib._end_suite(None, {})
    root = _run_root(tmp_path)
...
def test_redaction_applies_to_operation_arguments(tmp_path, monkeypatch):
    ...
    directly."""
    monkeypatch.setenv("RFDS_EVIDENCE_ROOT", str(tmp_path / "results"))
    run = evidence_module.EvidenceRun(driver_id="rf_picoscope2000a", activity="session")
...
def test_error_is_recorded_with_category(scope):
    lib, tmp_path = scope
    with pytest.raises(PicoScope2000AValidationError):
        lib.set_channel_enabled("Z", True)
    lib._end_suite(None, {})
    root = _run_root(tmp_path)
...
def test_evidence_disabled_writes_nothing_to_disk(tmp_path, monkeypatch):
    monkeypatch.setenv("RFDS_EVIDENCE_ROOT", str(tmp_path / "results"))
    lib = PicoScope2000ALibrary(evidence_enabled=False)
    lib.connect(simulated=True)
    lib.set_channel_enabled("A", True)
    lib._end_suite(None, {})
