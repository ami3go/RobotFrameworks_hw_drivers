...
(1 kHz, amplitude = 0.5 * channel range, see picoscope2000a.simulator)."""
...
from picoscope2000a.measurements import compute_measurements
from picoscope2000a.models import Waveform
from picoscope2000a.simulator import (
...
def test_get_measurement_raises_when_undeterminable_from_a_too_short_capture():
    ...
    d.capture_block()
    with pytest.raises(PicoScope2000AValidationError, match="could not be determined"):
...
def test_get_measurements_requires_a_capture():
    ...
    with pytest.raises(PicoScope2000AValidationError, match="no capture available"):
...
def test_compute_measurements_rejects_an_empty_waveform():
    ...
    with pytest.raises(PicoScope2000AValidationError, match="no samples"):
