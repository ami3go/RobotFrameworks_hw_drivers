...
from picoscope2000a.exceptions import PicoScope2000ADeviceError, PicoScope2000AValidationError
from picoscope2000a.simulator import SimulatedBus


@pytest.fixture
def driver():
    d = PicoScope2000A.connect_simulated()  # default simulated model is AWG-capable (2208B)
    yield d
    d.close()
...
def test_set_awg_waveform_raises_on_a_non_awg_capable_model():
    ...
    with pytest.raises(PicoScope2000ADeviceError, match="no built-in AWG"):
        driver.set_awg_waveform("sine", frequency_hz=1000.0, peak_to_peak_v=1.0)
