...
def test_capture_block_requires_timebase():
    ...
    with pytest.raises(PicoScope2000AValidationError, match="timebase has not been configured"):
...
def test_capture_block_requires_an_enabled_channel():
    ...
    with pytest.raises(PicoScope2000AValidationError, match="no channels are enabled"):
        driver.capture_block()
...
def test_get_waveform_rejects_unenabled_channel(driver):
    driver.capture_block()
    with pytest.raises(PicoScope2000AValidationError, match="was not enabled/captured"):
