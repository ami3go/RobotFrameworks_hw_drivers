"""Typed core driver for the PicoScope 2000A family (ps2000a).

Owns backend selection, autodetect, and identity. The Robot Framework
adapter (``rf_picoscope2000a/library.py``) is a thin layer on top of this
module and must not duplicate any of this logic.
"""
...
from .backend import Ps2000aBackend, RealPs2000aBackend
...
# ps2000a-family variants with a built-in AWG — matched against the model
# string ps2000aGetUnitInfo reports (e.g. "2208B", "2206 MSO"). This driver
# targets exactly this subset of the 2000 series (see package README).
_AWG_CAPABLE_MODEL_SUBSTRINGS = ("2205A", "2206", "2207", "2208", "2405A")
...
def _validate_edge(edge: str) -> bool:
    ...
    if normalized == "FALLING":
        return False
    raise PicoScope2000AValidationError(f"edge must be RISING or FALLING, got {edge!r}")
...
class PicoScope2000A:
    """A connected session with one PicoScope 2000A-family instrument."""

    def __init__(self, backend: Ps2000aBackend) -> None:
    ...
    # ------------------------------------------------------------------
    @classmethod
    def connect(cls, resource: str | None = None) -> PicoScope2000A:
        """Open real hardware. ``resource`` is a serial; omit it to autodetect."""

        return cls._connect_with_backend(RealPs2000aBackend(), resource)

    @classmethod
    def connect_simulated(
        cls, resource: str | None = None, bus: SimulatedBus | None = None
    ) -> PicoScope2000A:
        return cls._connect_with_backend(SimulatedBackend(bus), resource)

    @classmethod
    def _connect_with_backend(cls, backend: Ps2000aBackend, resource: str | None) -> PicoScope2000A:
        driver = cls(backend)
    ...
    def _open(self, resource: str | None) -> None:
        ...
        # Push our known-good channel defaults immediately: ps2000aSetChannel is
    ...
    def set_channel_probe(self, channel: str, probe_type: str, scale: float = 1.0) -> None:
        ...
        scale = float(scale)
        if scale <= 0:
            raise PicoScope2000AValidationError(f"scale must be positive, got {scale!r}")
    ...
    def _require_capture(self) -> _Capture:
        if self._last_capture is None:
            raise PicoScope2000AValidationError("no capture available; call 'capture_block' first")
    ...
    def _require_awg_capable(self) -> None:
        ...
        if not any(marker in model for marker in _AWG_CAPABLE_MODEL_SUBSTRINGS):
            raise PicoScope2000ADeviceError(
                f"connected model {model!r} has no built-in AWG "
                "(AWG-capable ps2000a models: 2205A MSO/2206B/2207B/2208B/2405A)"
            )
    ...
    def load_preset(self, path: str | Path) -> None:
        ...
        if data["awg"] is not None:
            ...
            try:
                self._require_awg_capable()
            except PicoScope2000ADeviceError:
                pass
            else:
                self.stop_awg()
