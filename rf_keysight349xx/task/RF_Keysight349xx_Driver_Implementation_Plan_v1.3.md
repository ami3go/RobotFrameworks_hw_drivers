...
**Date:** 2026-08-17  
**Supersedes:** v1.2; v1.0, v1.1 and v1.2 retained alongside this document
...
Reset Device:
  ...
  reset_type: device reset to default state
  output_behavior: opens all channel relays, owned and unowned
  settings: cleared to defaults
  reconnect_behavior: session preserved; routing state invalidated
  ready_time: to be established from the command reference
  safety_risk: >
    Overrides the §21 ownership model entirely. Not safe on a shared frame
    carrying another team's wiring.
  ownership_gate: >
...
- every RFDS citation names a version held in `AI_Guides/` (§2.1);
