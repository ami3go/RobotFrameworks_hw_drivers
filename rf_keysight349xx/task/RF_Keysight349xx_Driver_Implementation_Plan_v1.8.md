...
**Date:** 2026-08-17  
**Supersedes:** v1.7; v1.0–v1.7 retained alongside this document
...
1. **reference → map** — re-extract commands from
   ...
   absent from the map. This check **shall not reuse the map generator's extractor output as its
   expected set**: v1.6 asserted the map against the list its own generator produced, which proved
   only self-consistency and is how a false 100% coverage claim survived a commit.
...
**Counts.** ~155 public keywords total — ~121 device-facing across 347 dispositioned vendor
