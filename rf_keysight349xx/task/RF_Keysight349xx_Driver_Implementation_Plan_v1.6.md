...
**Date:** 2026-08-17  
**Supersedes:** v1.5; v1.0–v1.5 retained alongside this document
...
`protocol/vendor_command_coverage.yaml` is the **authoritative** binding between vendor commands and
public keywords. The table below is generated from it and lists all **112** planned keywords against
the **193** dispositioned vendor commands.

The narrative sections §9–§18 explain why each capability exists and what constrains it. Where a
narrative list and this table disagree, **this table governs** — the narrative sections predate the
vendor audit, and v1.5 shipped with 40 keywords present in the audit and absent from the prose.

Exact signatures, argument names, units, and return schemas are **not** fixed here; they are the
Phase 1 Gate 1 public API contract deliverable (§39). This table fixes the *set* and its device
...
| `Set Internal DMM State` | `INSTrument:DMM` |
...
| `Set Display Text` | `DISPlay:TEXT` |
| `Set Instrument Date` | `SYSTem:DATE` |
| `Set Instrument Time` | `SYSTem:TIME` |
...
| Keyword | Vendor command(s) |
|---|---|
| `Configure AC Voltage` | `[SENSe:]VOLTage:AC:BANDwidth` |
| `Configure DC Voltage` | `CONFigure:VOLTage:DC`, `INPut:IMPedance:AUTO`, `[SENSe:]VOLTage:DC:APERture`, `[SENSe:]VOLTage:DC:NPLC`, `[SENSe:]VOLTage:DC:RANGe`, `[SENSe:]VOLTage:DC:RANGe:AUTO`, `[SENSe:]VOLTage:DC:RESolution` |
| `Configure Frequency` | `[SENSe:]FREQuency:RANGe:LOWer` |
| `Configure Period` | `CONFigure:PERiod`, `[SENSe:]PERiod:APERture`, `[SENSe:]PERiod:VOLTage:RANGe`, `[SENSe:]PERiod:VOLTage:RANGe:AUTO` |
| `Get Channel Configuration` | `CONFigure?` |
| `Get Channel Function` | `[SENSe:]FUNCtion` |
| `Measure DC Voltage` | `MEASure:VOLTage:DC?` |
| `Measure Period` | `MEASure:PERiod?` |
| `Set Autozero` | `[SENSe:]ZERO:AUTO` |
| `Set Channel Function` | `[SENSe:]FUNCtion` |

**§11.3 — Current measurement (34901A ch 21-22)**

| Keyword | Vendor command(s) |
|---|---|
| `Configure AC Current` | `[SENSe:]CURRent:AC:BANDwidth` |
| `Configure DC Current` | `CONFigure:CURRent:DC`, `[SENSe:]CURRent:DC:APERture`, `[SENSe:]CURRent:DC:NPLC`, `[SENSe:]CURRent:DC:RANGe`, `[SENSe:]CURRent:DC:RANGe:AUTO`, `[SENSe:]CURRent:DC:RESolution` |
...
| `Configure Scan List` | `ROUTe:SCAN` |
| `Configure Trigger Source` | `ROUTe:CHANnel:ADVance:SOURce`, `TRIGger:SOURce` |
| `Fetch Readings` | `FETCh?` |
| `Get Last Reading` | `DATA:LAST?` |
| `Get Reading Count` | `DATA:POINts?` |
| `Get Scan List` | `ROUTe:SCAN` |
| `Get Scan List Size` | `ROUTe:SCAN:SIZE?` |
| `Get Scan Start Time` | `SYSTem:TIME:SCAN?` |
| `Get Trigger Count` | `TRIGger:COUNt` |
| `Get Trigger Source` | `TRIGger:SOURce` |
| `Get Trigger Timer` | `TRIGger:TIMer` |
| `Initiate Scan` | `INITiate` |
| `Read Buffered Readings` | `R?` |
| `Read Scan` | `READ?` |
| `Set Channel Delay` | `ROUTe:CHANnel:DELay`, `ROUTe:CHANnel:DELay:AUTO` |
| `Set Reading Count Threshold` | `DATA:POINts:EVENt:THReshold` |
| `Set Reading Format` | `FORMat:READing:ALARm`, `FORMat:READing:CHANnel`, `FORMat:READing:TIME`, `FORMat:READing:TIME:TYPE`, `FORMat:READing:UNIT` |
| `Set Trigger Count` | `TRIGger:COUNt` |
| `Set Trigger Timer` | `TRIGger:TIMer` |
| `Trigger Scan` | `*TRG` |
...
| Keyword | Vendor command(s) |
|---|---|
| `Configure Channel Monitor` | `ROUTe:MONitor` |
| `Read Monitor Data` | `ROUTe:MONitor:DATA?` |
| `Set Monitor State` | `ROUTe:MONitor:STATe` |
...
| Keyword | Vendor command(s) |
|---|---|
| `Configure RTD` | `[SENSe:]TEMPerature:TRANsducer:FRTD:RESistance[:REFerence]`, `[SENSe:]TEMPerature:TRANsducer:FRTD:TYPE` |
| `Configure Temperature` | `CONFigure:TEMPerature`, `[SENSe:]TEMPerature:APERture`, `[SENSe:]TEMPerature:NPLC`, `[SENSe:]TEMPerature:TRANsducer:TYPE` |
| `Configure Thermistor` | `[SENSe:]TEMPerature:TRANsducer:THERmistor:TYPE` |
| `Configure Thermocouple` | `[SENSe:]TEMPerature:TRANsducer:TCouple:CHECk`, `[SENSe:]TEMPerature:TRANsducer:TCouple:RJUNction`, `[SENSe:]TEMPerature:TRANsducer:TCouple:RJUNction:TYPE`, `[SENSe:]TEMPerature:TRANsducer:TCouple:TYPE` |
| `Get Reference Junction` | `[SENSe:]TEMPerature:RJUNction?` |
| `Measure Temperature` | `MEASure:TEMPerature?` |
| `Set Temperature Units` | `UNIT:TEMPerature` |

**§14.1 — Four-wire and resistance**

| Keyword | Vendor command(s) |
|---|---|
| `Configure 4 Wire Resistance` | `CONFigure:FRESistance`, `[SENSe:]FRESistance:APERture`, `[SENSe:]FRESistance:NPLC`, `[SENSe:]FRESistance:OCOMpensated`, `[SENSe:]FRESistance:RANGe`, `[SENSe:]FRESistance:RANGe:AUTO`, `[SENSe:]FRESistance:RESolution` |
| `Measure 4 Wire Resistance` | `MEASure:FRESistance?` |
| `Set Four Wire Mode` | `ROUTe:CHANnel:FWIRe` |

**§15.2 — Switching and routing**

| Keyword | Vendor command(s) |
|---|---|
| `Close Channel` | `ROUTe:CLOSe` |
| `Close Channel Exclusive` | `ROUTe:CLOSe:EXCLusive` |
| `Close Channels` | `ROUTe:CLOSe` |
| `Open Channel` | `ROUTe:OPEN` |
| `Open Channels` | `ROUTe:OPEN` |
...
| `Write Digital Byte` | `SOURce:DIGital:DATA[:{BYTE|WORD}]` |
| `Write Digital Word` | `SOURce:DIGital:DATA[:{BYTE|WORD}]` |
...
| `Get Totalizer Edge` | `[SENSe:]TOTalize:SLOPe` |
| `Read Totalizer` | `MEASure:TOTalize?`, `[SENSe:]TOTalize:DATA?` |
| `Set Totalizer Edge` | `[SENSe:]TOTalize:SLOPe` |
| `Set Totalizer Read Mode` | `[SENSe:]TOTalize:TYPE` |
...
| Keyword | Vendor command(s) |
|---|---|
| `Get DAC Voltage` | `SOURce:VOLTage` |
| `Set DAC Voltage` | `SOURce:VOLTage` |

**§17 — Alarms and scaling**

| Keyword | Vendor command(s) |
|---|---|
| `Clear Alarm` | `OUTPut:ALARm:CLEar:ALL` |
| `Configure Alarm Output` | `OUTPut:ALARm:MODE`, `OUTPut:ALARm:SLOPe`, `OUTPut:ALARm{1|2|3|4}:SOURce` |
| `Configure Digital Pattern Alarm` | `CALCulate:COMPare:DATA`, `CALCulate:COMPare:STATe`, `CALCulate:COMPare:TYPE` |
| `Configure Digital Pattern Mask` | `CALCulate:COMPare:MASK` |
| `Disable Scaling` | `CALCulate:SCALe:STATe` |
| `Enable Scaling` | `CALCulate:SCALe:STATe` |
| `Enable Upper Alarm` | `CALCulate:LIMit:UPPer:STATe` |
| `Get Alarm State` | `STATus:ALARm:CONDition?`, `STATus:ALARm[:EVENt]?`, `SYSTem:ALARm?` |
| `Set Scaling Offset` | `CALCulate:SCALe:OFFSet`, `CALCulate:SCALe:OFFSet:NULL` |
| `Set Scaling Unit` | `CALCulate:SCALe:UNIT` |
| `Set Upper Alarm Limit` | `CALCulate:LIMit:UPPer` |
...
| `Set LXI Identify` | `LXI:IDENtify[:STATe]` |

**§18.2 — 34972A USB and files**

| Keyword | Vendor command(s) |
|---|---|
| `Configure USB Log Format` | `MMEMory:FORMat:READing:CSEParator`, `MMEMory:FORMat:READing:RLIMit` |
| `Export Readings To USB` | `MMEMory:EXPort?` |
| `List USB Configuration Files` | `MMEMory:IMPort:CATalog?` |
| `Set Scan Logging To USB` | `MMEMory:LOG[:ENABle]` |

**Drift guard.** `validate_structure.py` shall fail when a keyword appears in
`vendor_command_coverage.yaml` but not in `api/public_api.yaml`, or the reverse, and when a `PUBLIC`
command has no bound keyword. This check is part of the §29.1 verification sequence.

**Counts.** 112 keywords / 193 commands. This is roughly double the keyword surface of the largest
existing driver in the repository, and the §38 twelve-phase estimate was made before the number was
known. Scope shall be re-assessed at Phase 1 Gate 5 against the actual Gate 1–5 effort.
...
24. Structure validation — `validate_structure.py` passes, including packaged-resource parity
    (§37.1) and keyword-inventory parity between `vendor_command_coverage.yaml` and
    `api/public_api.yaml` (§9.2)
...
- the keyword inventory in `api/public_api.yaml` matches `vendor_command_coverage.yaml` exactly (§9.2);
