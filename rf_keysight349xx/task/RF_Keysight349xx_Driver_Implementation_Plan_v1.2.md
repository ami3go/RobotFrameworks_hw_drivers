...
**Date:** 2026-08-17  
**Supersedes:** v1.1; v1.0 and v1.1 retained alongside this document
...
- RFDS-008 — Live Evidence and Run Record Standard
...
RFDS-008 governs the live evidence run this driver shall produce. Every driver already released in
the project repository implements an evidence engine, and §28 of this plan depends on its outputs,
so the standard is part of the baseline rather than an optional extra.

The evidence engine shall produce, per run:
...
- every public keyword call is recorded as a correlated operation with arguments, duration, and
  result or failure;
- `final_status` shall be derived from whether the run actually recorded an error, never hardcoded;
- every evidence run that is created shall also be finalized, including runs whose session was
  never opened or was deliberately left open;
- protocol capture shall attach before the connection is opened (see §28);
- `execution_mode` shall distinguish `SIMULATOR`, `REAL_HARDWARE`, `MIXED`, and `NO_HARDWARE`, and
  shall never report a simulator session as real hardware.
...
```yaml
RFDS-011: UNKNOWN         # disposition to be confirmed at Gate 1
RFDS-013: UNKNOWN         # capability authority; referenced by other drivers in the repository,
                          # so absence here is provisional and likely to become IN_SCOPE
RFDS-016: UNKNOWN         # disposition to be confirmed at Gate 1
```

No RFDS document shall be silently omitted from traceability.
...
```text
Source release version   (RELEASE_VERSION in version.py):  "26.01.00"
Distribution version     (project.version in pyproject):   "26.1.0"
```
...
- DMM presence and enabled state shall be discovered during connection (§6);
...
- **lock granularity** — session, mainframe, scan engine, and per-slot resources;
...
- raw I/O shall be **disabled by default** and shall remain disabled until `Enable Raw SCPI` is
  called with an explicit confirmation argument;
- `Enable Raw SCPI` shall be classified **high risk**; `Raw SCPI Write` shall be classified high
  risk; `Raw SCPI Query` shall be classified medium risk;
...
Reset Device:
  status: PUBLIC
  risk: HIGH
  gate: explicit confirmation argument required
  side_effect: opens all channel relays, owned and unowned
  ownership: overrides the §21 ownership model entirely

Preset Device:
  status: PUBLIC
  risk: HIGH
  gate: explicit confirmation argument required
  side_effect: to be established from the command reference before implementation
...
- both reset keywords shall require an explicit confirmation argument, in the manner §19 applies to
  service operations;
...
"Bounded iteration" shall have a stated bound. The driver shall define a maximum drain iteration
count in configuration (§23), with a documented default.
...
current_measurement:
  ...
  note: >
    DC and AC current measurement are available only on specific dedicated
    current channels of particular multiplexer cards, and are unavailable on
    others entirely. Record the exact current-capable channel set per card.

four_wire_resistance:
  status: SOURCE_VERIFICATION_REQUIRED
  note: See §14.1 -- pairing is card-dependent and reduces the usable channel set.
```
...
monitor_mode:
  status: SOURCE_VERIFICATION_REQUIRED
  note: >
    Contends with the scan engine for the internal DMM, so it interacts with
    both the §4.4 DMM gate and the §5.2 locking model. Determine whether it
    becomes PUBLIC or EXCLUDED before Phase 4.
```
...
four_wire_pairing:
  status: SOURCE_VERIFICATION_REQUIRED
  required_data:
    - per-card pairing offset
    - per-card usable channel set when a four-wire function is configured
```

This shall be recorded as capability-model data verified against the command reference. Channel
...
switching_topologies:
  ...
  classes:
    multiplexer:
      note: >
        Break-before-make within a bank -- closing one channel opens the
        previously closed one. A multi-channel close within one bank is not
        physically satisfiable and shall be rejected before transmission.
    actuator_general_purpose:
      note: Independent relays; simultaneous closure is valid.
    matrix:
      note: >
        Addressed by row/column crosspoint rather than a flat channel number.
        Requires a distinct addressing model or an explicit exclusion.
    rf_multiplexer:
      note: Distinct switching topology; disposition required.
```
...
```yaml
Get Digital Direction: UNKNOWN
```

The 34907A digital I/O channels are bidirectional, and direction is generally implicit in whether a
channel is read or written. This plan does not establish that a queryable direction register exists.

`Get Digital Direction` shall therefore not be implemented until the command reference establishes a
corresponding device command. Its disposition shall be recorded in
`protocol/vendor_command_coverage.yaml` before Phase 7 begins:

- if an authoritative command exists, the keyword is promoted to `PUBLIC` and implemented normally;
- if none exists, the disposition becomes `NOT_APPLICABLE` and the keyword is dropped.
...
The DAC output range shall be validated **before transmission**. The permitted range shall be
recorded as capability-model data verified against the command reference, and a request outside it
shall raise a local validation error rather than being transmitted and rejected by the device.
Consistent with §21's fail-closed posture, an unknown DAC range shall block DAC writes rather than
permit them optimistically.
...
Bounded retry may be allowed for operations that are demonstrably safe and idempotent — meaning the
operation changes no device state of any kind, including buffer and queue state.
...
├── validate_ai_contract.ps1
├── validate_ai_contract.sh
├── release_check.ps1
└── release_check.sh
```
...
11. Whether monitor mode becomes a public capability or is excluded (§12.1).
12. The topology class, current-capable channel set, and four-wire pairing offset of every supported
    card (§11.3, §14.1, §15.2).
13. The permitted DAC output range (§16.4).
14. Whether concurrent scan operations from different sessions are permitted or refused (§5.2).
15. The disposition of `Preset Device`'s exact side effect (§9.1).
