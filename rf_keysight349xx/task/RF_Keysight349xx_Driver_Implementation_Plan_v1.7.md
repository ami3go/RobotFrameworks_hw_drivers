...
**Date:** 2026-08-17  
**Supersedes:** v1.6; v1.0–v1.6 retained alongside this document
...
**Drift guard.** Two checks, guarding two different edges. v1.6 had only the first, and R1/R2 fell
through the second.

1. **map ↔ `api/public_api.yaml`** — fail when a keyword appears in one and not the other, or when a
   `PUBLIC` command has no bound keyword.
2. **reference ↔ map** — re-extract commands from
   `reference/Keysight_34970A_34972A_Command_Reference.md` and fail when any declared command is
   absent from the map.

Check 2 **shall not reuse the map generator's extractor output as its expected set.** The v1.6
generator asserted the map against the same list it had produced, which proved only self-consistency
and is how a false 100% coverage claim survived. An independent extraction is the requirement, not
an implementation detail.

Both checks are part of the §29.1 verification sequence.

**Counts.** 121 keywords / 347 commands. This is roughly double the keyword surface of the largest
