...
RFDS-008 defines the common evidence envelope, event schema, manifest, and storage rules shared across the platform; RFDS-019 remains authoritative for what must be verified and captured at the protocol boundary. The layout above is a conformance-specific view and should be reachable from, or nested under, the RFDS-008 §11 canonical result root for the same run rather than maintained as a wholly separate evidence tree.
