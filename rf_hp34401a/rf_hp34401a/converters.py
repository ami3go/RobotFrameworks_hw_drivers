...
def as_nplc(value: Any) -> Nplc:
    ...
    for member in Nplc:
        if member.value == number:
            return member
    raise ValueError("nplc must be one of 0.02, 0.2, 1, 10, 100")
...
def as_aperture(value: Any) -> Aperture:
    ...
    for member in Aperture:
        if member.value == number:
            return member
    raise ValueError("aperture must be one of 0.01, 0.1, 1 seconds")
...
def as_ac_filter(value: Any) -> AcFilterHz:
    ...
    for member in AcFilterHz:
        if member.value == number:
            return member
    raise ValueError("AC filter must be one of 3, 20, 200 Hz")
...
def as_autozero(value: Any) -> AutozeroMode:
    ...
    try:
        ...
        raise ValueError("autozero must be OFF, ONCE, or ON") from exc
...
def as_trigger_source(value: Any) -> TriggerSource:
    ...
    try:
        ...
        raise ValueError("trigger source must be IMMEDIATE, BUS, or EXTERNAL") from exc
...
def as_terminal(value: Any) -> InputTerminal:
    ...
    if text not in {"FRONT", "REAR"}:
        raise ValueError("terminal must be FRONT or REAR")
