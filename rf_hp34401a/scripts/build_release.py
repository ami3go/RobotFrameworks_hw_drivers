#!/usr/bin/env python3
"""Build and validate rf_hp34401a_v26.06.zip with one stable root."""
...
def validate_zip(output: Path) -> None:
    with zipfile.ZipFile(output) as archive:
        ...
        required = {
            ...
            f"{ROOT_NAME}/history/v26.06.md",
            f"{ROOT_NAME}/review/v26.06_release_readiness.md",
        }
...
def generate_internal_checksums(root: Path) -> Path:
    """Hash every distributable source file except the checksum index itself."""
...
def main() -> int:
    ...
    if not args.skip_quality_gates:
        run([sys.executable, "-m", "pytest", "--cov=rf_hp34401a", "--cov-fail-under=95"], root)
    ...
    if wheel is None or sdist is None:
        raise SystemExit("Expected wheel and source distribution were not built")
