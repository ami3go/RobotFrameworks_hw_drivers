"""RFDS-008 evidence and diagnostics engine for :mod:`rf_phidget_relay`.

Every public keyword call is recorded as a *run* under ``results/session/
rf_phidget_relay/<timestamp>_<run_id>/`` (override the root with the
``RFDS_EVIDENCE_ROOT`` environment variable): one JSON Lines entry per
event/operation/error, a redacted argument record, the underlying Phidget22
SDK calls as a protocol trace, a device-identity snapshot, and, once
``Disconnect Relays`` runs, a ``run_summary.json`` + ``evidence_manifest.json``
pair with a SHA-256 hash per artifact. ``Export Diagnostic Bundle`` zips the
whole thing up for sending to whoever is troubleshooting a failure.

This module intentionally implements the parts of RFDS-008 that make a
concrete troubleshooting difference for a small relay driver — structured,
correlated, redacted evidence with integrity hashes — and does not attempt
platform-scale concerns the standard also describes (log rotation/backpressure
policy, crash-recovery tooling, cryptographic signing, retention/archival
automation, a cross-driver shared package). Those are noted inline where
...
rf_phidget_relay.evidence.EvidenceListener`` to correlate events with the
...
logger = logging.getLogger("rf_phidget_relay.evidence")
...
def _classify_exception(exc: BaseException) -> str:
    ...
    maps the driver's own exception names (``rf_phidget_relay/exceptions.py``)
    to a small stable category set instead of inventing RFDS-007 codes.
    """
    name = type(exc).__name__
    if "Dependency" in name:
        return "ENVIRONMENT"
    if "Configuration" in name or "Validation" in name or "ArgumentType" in name:
        return "VALIDATION"
    if "State" in name:
        return "STATE"
    if "Hardware" in name:
...
def _safe_call(obj: Any, method_name: str) -> Any:
    """Best-effort vendor SDK query; ``None`` (never a raised exception) when unavailable."""
    try:
...
class EvidenceRun:
    ...
    def __init__(
        ...
        *,
        ...
        # issued" and "the Nth line written to this stream" previously produced
        # gaps whenever an ID was issued before its record was appended.
    ...
    def _write_environment(self) -> None:
        payload = {
            ...
            "driver_distribution_version": _safe_distribution_version("rf-phidget-relay"),
            "driver_source_version": _safe_module_version("rf_phidget_relay"),
            "phidget22_distribution_version": _safe_distribution_version("Phidget22"),
    ...
    def emit_event(
        ...
        *,
        ...
        record = {
            ...
            "schema_version": SCHEMA_VERSION,
            "run_id": self.run_id,
            "producer_id": f"rf_phidget_relay:{session_alias or self.driver_id}",
            "timestamp_utc": _utc_now_iso(),
    ...
    def log_protocol(
        ...
        *,
        ...
        """Record one outbound/inbound Phidget22 SDK call as protocol evidence.
        ...
        (Phidget22 is a vendor SDK, not a byte-level transport this driver
    ...
    def _write_markdown_summary(self, summary: dict) -> None:
        lines = [
            ...
            "for the underlying Phidget22 SDK calls, in the order they happened.",
            "",
        ]
...
class EvidenceListener:
    """Register with ``--listener rf_phidget_relay.evidence.EvidenceListener``.
