...
- Added `Set Fan`/`Get Fan` keywords and a `settings.auxiliary_outputs.fan_output_channel` setting (also accepted as a `Connect` option), following the existing dryer/compressed-air auxiliary-output pattern: the fan is driven as a SimServ digital output, defaults to `null`, and stays capability-gated with `DriverUnsupportedOperationError` until its physical channel is qualified. `Safe Shutdown` gains a `fan_off` action, `SKIP`ped while unconfigured. Canonical keyword surface 58 → 60. Note some chamber models regulate ventilation as a percentage setpoint rather than a binary output; these keywords cover digital-output wiring only.
- Generalized the auxiliary-output channel-collision check to all three outputs, at both the core (`DriverLimitViolationError` on connect) and configuration-validation layers.
- Fixed `examples/13_safe_shutdown.robot` asserting `${result}[dryer]`/`${result}[compressed_air]`, which `Safe Shutdown` has never returned — it reports per-output outcomes under `actions`. The example failed for every driver version that shipped it; `scripts/run_all_examples.py` is not part of the pytest suite, so nothing caught it.
