...
...               channel and all channels), AWG control, a preset save/load round trip,
...               and connecting to a specific SDK series (2000A by default, 3000A here).
...
Connect To A 3000A Series Simulated Device
    [Documentation]    series selects which SDK/native driver a real connection would use;
    ...    each series has its own default simulated device and AWG-capable model list.
    Connect    alias=scope3000    simulated=${TRUE}    series=3000A
    ${identity}=    Get Identity    alias=scope3000
    Should Contain    ${identity}    3405D MSO
    Set Channel Enabled    A    ${TRUE}    alias=scope3000
    Set AWG Waveform    sine    frequency_hz=1000    peak_to_peak_v=1.0    alias=scope3000
    [Teardown]    Disconnect    alias=scope3000

Find Devices For A Specific Series
    ${devices}=    Find Devices    series=3000A    simulated=${TRUE}
    List Should Contain Value    ${devices}    SIM/00001
