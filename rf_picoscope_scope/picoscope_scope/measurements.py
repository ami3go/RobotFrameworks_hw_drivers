...
No supported series' ``picosdk`` module has an on-device "immediate
measurement" call — unlike a bench scope with its own display/firmware
(e.g. this repo's ``rf_tbs1000c``, which wraps Tektronix's
``MEASUrement:IMMed:TYPe``), every PicoScope SDK driver only ever gives you
raw ADC block data. Pico's own PicoScope desktop application
...
def compute_measurements(waveform: Waveform) -> Measurements:
    ...
    if not values:
        raise PicoScopeValidationError("waveform has no samples to measure")
