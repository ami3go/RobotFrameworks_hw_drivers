"""RFDS-008 evidence and diagnostics engine for :mod:`rf_picoscope_scope`.

Every public keyword call is recorded as a *run* under ``results/session/
rf_picoscope_scope/<timestamp>_<run_id>/`` (override the root with the
``RFDS_EVIDENCE_ROOT`` environment variable): one JSON Lines entry per
event/operation/error, a redacted argument record, a device-identity
snapshot, and, once the suite ends (or a session is explicitly finalized), a
...
Adapted near-verbatim from the evidence engine originally built for
rf_phidget_relay — this driver wraps a vendor SDK (``picosdk``, one module
per series: ``ps2000a``, ``ps3000a``, ...) the same way rf_phidget_relay
wraps Phidget22, not a text transport like
rf_tbs1000c's SCPI/USBTMC, so :meth:`EvidenceRun.log_protocol` frames the
same way: there is no wire protocol to trace, the SDK call itself is the
transmitted operation. This module intentionally implements the parts of
RFDS-008 that make a concrete troubleshooting difference for this driver —
structured, correlated, redacted evidence with integrity hashes — and does
not attempt platform-scale concerns the standard also describes (log
...
noted inline where relevant rather than silently skipped.

Nothing here talks to Robot Framework directly except :class:`EvidenceListener`,
which is optional (register with ``--listener
rf_picoscope_scope.evidence.EvidenceListener`` to correlate events with the
active suite/test/keyword); the engine itself works the same under plain
pytest.
"""
...
logger = logging.getLogger("rf_picoscope_scope.evidence")
...
def redact_mapping(data: Mapping[str, Any]) -> dict:
    ...
    credential-bearing arguments today, but keyword arguments are logged
...
def _classify_exception(exc: BaseException) -> str:
    ...
    RFDS-007"; this maps the core driver's own exception names
    (``picoscope_scope/exceptions.py``) to a small stable category set instead
    of inventing RFDS-007 codes.
    """
...
class EvidenceRun:
    """Owns one RFDS-008 result directory for one driver session.

    Not thread-safe across concurrent *sessions* (each session should own its
    ...
    def __init__(
        ...
        *,
        ...
        # Deliberately empty, not {execution_mode}: the constructor default is
        # just a placeholder until the first real note_execution_mode() call —
        # seeding it here would make every run register as "MIXED" the moment
        # a single real Connect (or simulated one) reported its actual mode.
    ...
    def note_execution_mode(self, mode: str) -> None:
        """Called once per session Connect; upgrades ``execution_mode`` to ``"MIXED"``
        if this run ends up spanning more than one distinct mode (e.g. one simulated
        alias and one real-hardware alias) — reflected in ``run_summary.json`` at
        :meth:`finalize`, not retroactively in the already-written ``environment.json``."""

        self._execution_modes_seen.add(mode)
        self.execution_mode = mode if len(self._execution_modes_seen) == 1 else "MIXED"
    ...
    def _write_environment(self) -> None:
        payload = {
            ...
            "driver_distribution_version": _safe_distribution_version("robotframework-picoscope-scope"),
            "driver_source_version": _safe_module_version("rf_picoscope_scope"),
            "picosdk_distribution_version": _safe_distribution_version("picosdk"),
    ...
    def emit_event(
        ...
        *,
        ...
        record = {
            ...
            "schema_version": SCHEMA_VERSION,
            "run_id": self.run_id,
            "producer_id": f"rf_picoscope_scope:{session_alias or self.driver_id}",
            "timestamp_utc": _utc_now_iso(),
    ...
    def log_protocol(
        ...
        *,
        ...
        """Record one outbound/inbound ``picosdk`` SDK call as protocol evidence.

        ``direction`` is ``"outbound"`` (driver -> SDK/device) or ``"inbound"``
        (SDK/device -> driver). This is the RFDS-019/RFDS-008 protocol-boundary
        observation point for this driver: there is no wire protocol to trace
        (``picosdk`` is a vendor SDK, not a byte-level transport this driver
        controls), so the SDK call itself — function and arguments — is the
        transmitted operation, exactly as RFDS-019 §6.3 lists "vendor SDK
        function call" as a supported outbound operation type.
        """
    ...
    def finalize(self, status: str = "PASS") -> Path:
        ...
        Idempotent — a second call is a no-op and returns the same root.
        """
        if self._finalized:
            return self.root
    ...
    def _write_markdown_summary(self, summary: dict) -> None:
        lines = [
            ...
            "for the underlying picosdk SDK calls, in the order they happened.",
            "",
        ]
...
class NullEvidenceRun:
    ...
    @contextlib.contextmanager
    def record_operation(self, capability: str, *, arguments: Optional[Mapping[str, Any]] = None, session_alias=None):
...
class EvidenceListener:
    """Register with ``--listener rf_picoscope_scope.evidence.EvidenceListener``.
    ...
    engine works without it, just without Robot-side correlation.
    """
