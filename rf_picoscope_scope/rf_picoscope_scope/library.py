"""Robot Framework adapter for :mod:`picoscope_scope`.
...
from picoscope_scope import PicoScope
...
def _as_bool(value: Any, name: str = "value") -> bool:
    ...
    if isinstance(value, str):
        ...
        if normalized in {"false", "0", "no", "off", "", "none"}:
            return False
    raise PicoScopeValidationError(f"{name} must be a Boolean, got {value!r}")
...
def _evidenced(func):
    ...
    *below* ``@keyword(...)`` in the decorator stack (closest to ``def``).
    ...
    def wrapper(self: PicoScopeLibrary, *args: Any, **kwargs: Any):
...
class PicoScopeLibrary:
    """Robot Framework keywords for PicoScope oscilloscopes, across multiple SDK
    series (``series="2000A"`` by default, ``"3000A"`` also supported — see
    ``Connect``).
    ...
    def __init__(self, evidence_enabled: Any = True) -> None:
        self._sessions: dict[str, PicoScope] = {}
    ...
    def _ensure_evidence(self) -> Any:
        ...
        if self._evidence is None:
            if self._evidence_enabled:
                self._evidence = _evidence.EvidenceRun(driver_id="rf_picoscope_scope", activity="session")
            else:
    ...
    def _end_suite(self, name: str, attributes: dict[str, Any]) -> None:
        ...
        for alias in list(self._sessions):
            try:
                ...
                _rf_logger.warn(f"PicoScope: cleanup for {alias!r} reported: {exc}")  # noqa: G010
    ...
    def _session(self, alias: str | None = None) -> PicoScope:
        selected = self._resolve_alias(alias)
        if selected is None:
            raise PicoScopeConnectionError("no PicoScope connection is active; call 'Connect' first")
        try:
            return self._sessions[selected]
        except KeyError as exc:
            known = ", ".join(sorted(self._sessions)) or "none"
            raise PicoScopeConnectionError(
                f"unknown PicoScope alias {selected!r}; known aliases: {known}"
            ) from exc

    def _connection_state(self, alias: str, driver: PicoScope) -> dict[str, Any]:
    ...
    def connect(
        ...
        **options: Any,
        ...
        raises if zero or more than one are found. ``series`` selects which
        SDK/native driver to use (``"2000A"`` for ``picosdk.ps2000a``,
        ``"3000A"`` for ``picosdk.ps3000a``) — real hardware from different
        series can't be told apart without picking the right one, since each
        is a separate native driver with its own enumerate/open calls.
        Idempotent when ``alias`` is already connected to the same
        ``resource``.
        """
        ...
        driver = (
            PicoScope.connect_simulated(resource, series=series)
            if simulated
            else PicoScope.connect(resource, series=series)
        )
        ...
        self._active_alias = selected_alias
        _rf_logger.info(f"PicoScope: connected alias={selected_alias!r} resource={driver.resource!r}")
        state = self._connection_state(selected_alias, driver)
        evidence_run.record_device_identity(
            session_alias=selected_alias,
            manufacturer="Pico Technology",
            model_family=f"PicoScope {driver.series}",
    ...
    @_evidenced
    def find_devices(
        self, series: str = "2000A", simulated: bool = False, alias: str | None = None
    ) -> list[str]:
        ...
        (any real or simulated session can enumerate, regardless of its own
        series); otherwise probes the given ``series`` with a throwaway
        backend — pass ``simulated=${TRUE}`` to probe the bundled simulator
        instead of real hardware in that case.
        """

        selected = self._resolve_alias(alias)
        if selected is not None and selected in self._sessions:
            return self._sessions[selected].enumerate_devices()
        return PicoScope.enumerate_devices_for_series(series, _as_bool(simulated, "simulated"))
    ...
    def save_channel_image(
        ...
        Waveform`` data with matplotlib. Requires the ``rf_picoscope_scope[plot]`` extra."""
