...
def parse_idn(response: str) -> InstrumentIdentity:
    parts = [part.strip() for part in response.strip().split(",", 3)]
    if len(parts) != 4:
        raise N6700CommandError(f"invalid *IDN? response: {response!r}")
    manufacturer = " ".join(parts[0].upper().split())
    if manufacturer not in SUPPORTED_MANUFACTURERS:
        raise N6700CommandError(f"unsupported manufacturer in *IDN? response: {parts[0]!r}")
    return InstrumentIdentity(parts[0].strip(), parts[1].strip(), parts[2].strip(), parts[3].strip())
...
def parse_error(response: str) -> ScpiErrorRecord:
    ...
    return ScpiErrorRecord(code, message.strip().strip('"'), raw)


def parse_ieee488_definite_block(data: bytes) -> bytes:
    if not data.startswith(b"#"):
        raise N6700CommandError("binary block does not start with '#'")
    if len(data) < 2 or not chr(data[1]).isdigit():
        raise N6700CommandError("binary block has invalid length-of-length")
    n_digits = int(chr(data[1]))
    if n_digits == 0:
        raise N6700CommandError("indefinite-length binary block is not supported")
    header_len = 2 + n_digits
    if len(data) < header_len:
        raise N6700CommandError("truncated binary block header")
    try:
        payload_len = int(data[2:header_len].decode("ascii"))
    except ValueError as exc:
        raise N6700CommandError("binary block has non-numeric length") from exc
    end = header_len + payload_len
    if len(data) < end:
        raise N6700CommandError("truncated binary block payload")
